...
> **What's new in 0.2** — copy/paste and duplicate nodes (`Ctrl+C`/`Ctrl+V`), group nodes into colored frames (`Ctrl+G`), value previews under every node, library favourites (star any node) and tooltips, a recent-files menu on **Open**, resizable Watch nodes — and the node library has grown past **200 nodes** (clash triage and reports, camera control, bulk search sets, model audits, and 43 new general-purpose nodes).
