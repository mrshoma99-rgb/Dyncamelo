...
| **Implemented (v0.4)** | New in the v0.4 "user feedback" release: `Selection.Resolve` plus a shared `resolveTo` input on the seven `Search.*` picker nodes and `Selection.Current` (see [WHATS_NEW_0.4.md](WHATS_NEW_0.4.md)). |
...
| List.ReplaceNulls | List | list, substitute: object | list | Replace nulls at every nesting level with a substitute (0, "n/a") — keeps lengths/alignment where List.Clean would shift them. | recursive C# | Implemented (v0.31) |
| List.AllIndicesOf | List | list, item: object | indices: List&lt;int&gt; | Every index of an item (List.IndexOf finds only the first). | value equality scan | Implemented (v0.31) |
| List.LastIndexOf | List | list, item: object | index: int | Index of the LAST occurrence (-1 when absent). | reverse scan | Implemented (v0.31) |
| List.ReplaceItemAtIndex | List | list, index: int (negative = from end), item: object | list | New list with the element at the index replaced. | copy + assign | Implemented (v0.31) |
| List.Insert | List | list, item: object, index: int (negative = from end) | list | New list with the value inserted at the index. | copy + insert | Implemented (v0.31) |
| List.AddItemToFront | List | list, item: object | list | Prepend — the front twin of List.AddItemToEnd. | copy + prepend | Implemented (v0.31) |
...
| ModelItem.AncestorNameMatches | Navisworks.ModelItem | item: ModelItem, text: string, mode: choice = "contains" (contains / doesn't contain / starts with / doesn't start with / ends with / doesn't end with), includeSelf: bool = false, caseSensitive: bool = false | matches: bool, ancestor: ModelItem, ancestorName: string | Tests ancestor names (nearest first) against the text — the branch-membership filter ("under the Steel branch?", "NOT under *-DEMO?"). "Doesn't" modes are true when NO ancestor matches. Lace for a FilterByBoolMask mask; the matching ancestor comes out too. | walk `ModelItem.Parent` chain + string compare (tree-shown names, case-insensitive by default) / [MultiReturn] | Implemented (v0.29) |
| ModelItem.AncestorPropertyMatches | Navisworks.ModelItem | item: ModelItem, category: string, property: string, text: string, mode: choice = "contains" (same six as AncestorNameMatches), includeSelf: bool = false, caseSensitive: bool = false | matches: bool, ancestor: ModelItem, value: string | The general form of AncestorNameMatches: test ANY property of the ancestors ("Source File contains STEEL?", "Level Name starts with B?"). Ancestors without the property simply don't match; "doesn't" modes are true when NO ancestor matches. | parent-chain walk + `FindPropertyByDisplayName/ByName` + string compare / [MultiReturn] | Implemented (v0.31) |
...
> **Shared `resolveTo` input (v0.4):** every picker row below carries an optional `resolveTo: string = "Self"` input (just before `document`) that re-resolves the raw hits to another selection-tree level — `Self`, `File`, `Layer`, `FirstObject`, `LastObject`, `LastUnique` or `Geometry` — exactly like `Selection.Resolve` (which see, under [Navisworks.Selection](#navisworksselection)). Parsing is case-insensitive and ignores spaces/hyphens/underscores; an unknown level fails fast before the search runs.

| Node | Category | Inputs (name: type) | Outputs (name: type) | Description | Maps to API | Tier |
|---|---|---|---|---|---|---|
| Search.ByPropertyValue | Navisworks.Search | category: string, property: string, value: string, resolveTo: string = "Self", document: Document | modelItems: List&lt;ModelItem&gt; | Find every item whose property equals the value — the bulk-selection workhorse (equivalent to Find Items → Equals). | `new Search()`; `Search.Selection.SelectAll()`; `SearchConditions.Add(SearchCondition.HasPropertyByDisplayName(cat, prop).EqualValue(VariantData.FromDisplayString(value)))`; `Search.FindAll(doc, false)` | MVP |
| Search.ByPropertyContains | Navisworks.Search | category: string, property: string, value: string, resolveTo: string = "Self", document: Document | modelItems: List&lt;ModelItem&gt; | Find items whose property display-string contains the text. | `SearchCondition...DisplayStringContains(value)` + `Search.FindAll` | MVP |
| Search.ByPropertyWildcard | Navisworks.Search | category: string, property: string, pattern: string, resolveTo: string = "Self", document: Document | items: List&lt;ModelItem&gt; | Wildcard match (`*`, `?`) on property display string. | `SearchCondition...DisplayStringWildcard(pattern)` | Implemented (v0.2) |
| Search.ByPropertyCompare | Navisworks.Search | category: string, property: string, comparison: string, value: double, resolveTo: string = "Self", document: Document | items: List&lt;ModelItem&gt; | Numeric comparison search: &gt;, &gt;=, &lt;, &lt;= (e.g. pipes with Diameter &gt; 100). Runs once per numeric variant type and unions the hits. | `SearchCondition...CompareWith(SearchConditionComparison.Numeric*, VariantData.FromDouble/Length/Area/Volume/Angle/Int32)` | Implemented (v0.2) |
| Search.HasProperty | Navisworks.Search | category: string, property: string, resolveTo: string = "Self", document: Document | items: List&lt;ModelItem&gt; | All items that carry the property at all (data-completeness audits; see Audit.MissingProperty for the inverse). | `SearchCondition.HasPropertyByDisplayName(cat, prop)` (no value test) | Implemented (v0.2) |
| Search.HasCategory | Navisworks.Search | category: string, resolveTo: string = "Self", document: Document | items: List&lt;ModelItem&gt; | All items carrying a property tab (e.g. every item with "TimeLiner" data). | `SearchCondition.HasCategoryByDisplayName(cat)` | Implemented (v0.2) |
| Search.InItems | Navisworks.Search | modelItems: List&lt;ModelItem&gt;, category: string, property: string, value: object, resolveTo: string = "Self", document: Document | items: List&lt;ModelItem&gt; | Scoped search: run the property-equals test only inside the given items (chained refinement). | `Search.Selection.CopyFrom(ModelItemCollection)` + conditions + `FindAll` | Implemented (v0.2) |
...
| Node | Category | Inputs (name: type) | Outputs (name: type) | Description | Maps to API | Tier |
|---|---|---|---|---|---|---|
| Selection.Current | Navisworks.Selection | resolveTo: string = "Self", document: Document | modelItems: List&lt;ModelItem&gt; | Items currently selected in the Navisworks UI (bridge from manual picking into the graph). `resolveTo` re-resolves the picks to another selection-tree level (see Selection.Resolve). | `Document.CurrentSelection.SelectedItems` (ModelItemCollection) | MVP |
| Selection.Resolve | Navisworks.Selection | modelItems: List&lt;ModelItem&gt;, level: string = "LastObject" | items: List&lt;ModelItem&gt; | Maps every item to its representative at a selection-resolution level, like changing Options &gt; Interface &gt; Selection &gt; Resolution and re-picking. Levels: `Self` (pass-through), `File` (the model file node), `Layer` (containing layer, file node when the model has no layers), `FirstObject` (outermost composite/insert object below files/layers/collections), `LastObject` (innermost composite/insert object — Navisworks' default resolution), `LastUnique` (deepest path item that is not multiply instanced — `ModelItem.Instances`), `Geometry` (expands DOWN to the geometry leaves). Level parsing is case-insensitive and ignores spaces/hyphens/underscores. Output is deduplicated, input order preserved. | `ModelItem.Parent` path walk + `IsLayer`/`IsCollection`/`IsComposite`/`IsInsert`/`HasGeometry`/`DescendantsAndSelf` | Implemented (v0.4) |
...
| Viewpoints.ExportFile | Navisworks.Viewpoints | filePath: string, viewpoints: viewpoint/folder/name/list = all, document: Document | filePath: string, count: int, report: string | Export saved viewpoints — camera, section box, folder location — to a portable JSON package that Viewpoints.ImportFile rebuilds in ANOTHER model (element overrides are model-bound and deliberately not carried). | `Viewpoint.GetCamera()` (verbatim camera) + explicit camera fields + `InternalClipPlanes.GetOrientedBox` (box capture, Navisworks 2024) → `Dyncamelo.Nodes.Portable.ViewpointPackageFile` | Implemented (v0.26) |
| Viewpoints.ImportFile | Navisworks.Viewpoints | filePath: string, folderName: string = "" (recreate original folders; "A/B" nests), overwrite: bool = true, document: Document | viewpoints: List&lt;SavedViewpoint&gt;, count: int, report: string | Rebuild the packaged viewpoints in the current model — cameras, section boxes and folder structure, with automatic unit conversion when the source model used different units. Re-runs replace same-named views. | `UnitConversion.ScaleFactor` + `Viewpoint.TrySetCamera` (bit-perfect when units match) / camera fields + `InternalClipPlanes.SetBox/SetOrientedBox` + `SavedViewpoints.AddCopy/ReplaceWithCopy` | Implemented (v0.26) |
...
| Clash.GroupResults | Navisworks.Clash | results: List&lt;ClashResult&gt;, groupName: string, moveExisting: bool = false, document: Document | test, group, added: int, moved: int, skipped: int | Put an explicit result list into a named group — YOUR grouping rule, not a built-in one. Results already in other groups are skipped unless moveExisting pulls them in; an existing same-named group is extended, re-runs are clean. | copy → rebuild result tree (untouched groups carried as whole copies) → `TestsEditTestFromCopy` / [MultiReturn] | Implemented (v0.27) |
| ClashGroup.ByName | Navisworks.Clash | test: ClashTest (or name), groupName: string, document: Document | group, results: List&lt;ClashResult&gt;, status: string, count: int | Fetch a result group by test + group name and open it up: members, the group's own status, count. Error lists the test's actual group names. | find `ClashResultGroup` in `ClashTest.Children` + `.Status` / [MultiReturn] | Implemented (v0.27) |
| ClashTest.Groups | Navisworks.Clash | test: ClashTest (or name), document: Document | groups, names, statuses, counts (index-aligned) | All result groups of a test with each group's own status and size — the triage overview. | scan `ClashTest.Children` for `ClashResultGroup` / [MultiReturn] | Implemented (v0.27) |
| Clash.AllGroups | Navisworks.Clash | document: Document | groups, names, testNames, counts (index-aligned) | EVERY result group of every clash test as one flat list — the whole-project entry point: wire into Loop.Item, no List@Level needed (ClashTest.Groups' object port cannot lace over a test list). | walk all tests' `ClashResultGroup` children / [MultiReturn] | Implemented (v0.32) |
| ClashGroup.Info | Navisworks.Clash | group: ClashResultGroup | results, name, status, count, test, testName | Opens a group object up — its results, name, status and the TEST it belongs to. Makes group workflows one flat loop instead of parallel name lists. | `ClashResultGroup.Children` + parent walk to `ClashTest` / [MultiReturn] | Implemented (v0.32) |
| ClashResult.Orientation | Navisworks.Clash | result: ClashResult | degrees: double, shape1/shape2: string, slope1/slope2: double | ClashResult.Angle with world context: crossing angle PLUS each element's box shape (slab/wall/riser/run/block) and slope from horizontal — tells pipe-through-wall from pipe-through-floor at the same 90°. | bounding-box extents → pure `BoxOrientation` core (Dyncamelo.Nodes.Spatial) / [MultiReturn] | Implemented (v0.27) |
| Clash.FilterByOrientation | Navisworks.Clash | results: List&lt;ClashResult&gt;, shape1: choice = "any", shape2: choice = "any" | results | Keep only clashes between elements of the given box shapes, matched in either order — run+wall = pipes through walls, run/riser+slab = pipes through floors; angle filtering alone can't tell those apart. | per-item `BoxOrientation.Classify` + unordered pair match | Implemented (v0.27) |
| ClashResult.Focus | Navisworks.Clash | results: ClashResult or list, isolate: bool = true, zoom: bool = true, select: bool = false, paddingFactor: double = 1.5, document: Document | items: List&lt;ModelItem&gt; | Focus the view on clash result(s) like double-clicking in Clash Detective: isolate the pair(s), zoom to them, optionally select. Appearance.ShowAll undoes the isolate. | composes `Appearance.Isolate` + `Camera.ZoomToItems` + `Selection.SetCurrent` | Implemented (v0.27) |
| ClashTest.Name | Navisworks.Clash | test: ClashTest | name: string | The test's display name as a single output — for wiring into reports, group lookups and file paths. | `SavedItem.DisplayName` | Implemented (v0.27) |
| Clash.Status | Navisworks.Clash | status: choice = "New" | status: string | A clash status from a dropdown (New/Active/Reviewed/Approved/Resolved) — feeds ClashResult.SetStatus, Clash.FilterByStatus, ClashTest.ResultsByStatus without typing. | `[NodeChoices]` value node | Implemented (v0.27) |
| Clash.Statuses | Navisworks.Clash | newStatus: bool = true, active/reviewed/approved/resolved: bool = false | statuses: string | MULTI-select statuses with toggles → comma text ("New,Active") that Clash.FilterByStatus and ClashTest.ResultsByStatus accept. | bool toggles → joined text | Implemented (v0.31) |
| Clash.FilterByItemProperty | Navisworks.Clash | results, category: string, property: string, value1: string, value2: string = "" (any), mode: choice = "contains" (contains/equals/starts with/ends with), searchAncestors: bool = true, caseSensitive: bool = false | results | The semantic clash rule: keep clashes whose two items' property values match a text pair in either order ("Category contains Pipe vs contains Wall"). Property read from the item or its ancestors (clash items are often geometry leaves). | `PropertyCategories.FindPropertyByDisplayName/ByName` up the parent chain → pure `TextPairFilter` core | Implemented (v0.30) |
| Clash.FilterBySet | Navisworks.Clash | results, set: set/name/items, which: choice = "either" (either/both/item1/item2), invert: bool = false, document: Document | results | Keep clashes whose items belong to a selection/search set (scopes), or with invert drop them (ignore-lists: "Accepted penetrations"). Membership includes ancestors, matching how selections include descendants. | resolve set → `HashSet<ModelItem>` + parent-chain membership | Implemented (v0.30) |
| Clash.FilterByDepth | Navisworks.Clash | results, minDepth: double = 0, maxDepth: double = ∞, units: choice = "document", document: Document | results | Keep clashes by penetration depth in a named unit — minDepth 0.025 Meters drops every grazing clash under 25 mm. Depth = −Distance (positive overlap; clearance gaps read negative). | `ClashResult.Distance` negated + `UnitConversion.ScaleFactor` | Implemented (v0.30) |
| Clash.Deduplicate | Navisworks.Clash | results | results, duplicates | One clash per unique unordered item pair — multi-point pairs and mirrored A-vs-B / B-vs-A matrix duplicates count once; first in input order is the representative. | pair key from item identity (InstanceGuid, else tree path — the snapshot scheme) / [MultiReturn] | Implemented (v0.30) |
...
| ClashTest.ResultsByStatus | Navisworks.Clash | test: ClashTest, status: string ("New" or comma-multi "New,Active") | results: List&lt;ClashResult&gt; | The results of a test with the given status(es) — one, or several comma-separated (Clash.Statuses builds the text from toggles). | flatten `ClashTest.Children`, filter `ClashResult.Status` | Implemented (v0.2; multi v0.31) |
...
| Implemented (v0.4) | 1 (Selection.Resolve; plus the `resolveTo` input added to the 7 Search pickers and Selection.Current) |
| Beta (not yet implemented) | 8 |
| Skipped | 1 |
| Future | 7 (in-catalog) + 5 platform backlog |
| **Total catalog** | **262** |

Registered node count as built (v0.4): **107** general (`dyncamelo list-nodes`, includes the interactive Input/Output nodes) + **148** Navisworks zero-touch definitions = **255 nodes**. A few v0.1 nodes shipped under slightly different names than their catalog rows (`TimelinerTask.Info`/`Items` cover the `TimelinerTask.Name`/`Dates`/`Type`/`AttachedItems` rows; `Export.ToCsv` covers `Export.PropertyReportCsv`; plus `JSON.ReadFromFile`, `JSON.WriteToFile`, `BoundingBox.ByCorners`, `Units.Current`, `TimelinerTask.Create` beyond the catalog).
