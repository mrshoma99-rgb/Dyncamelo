...
This catalog defines the complete planned node library for Dyncamelo, the visual programming environment for Autodesk Navisworks 2024. It is the product-design source of truth for node names, ports, behavior, and the exact Navisworks API surface each node wraps.
...
MVP node checklist (88 nodes) — names grouped by category:

**Input (7):** Number, Number Slider, Integer Slider, String, Boolean, File Path, Directory Path
**Output (3):** Watch, Watch List, Note
**Math (8):** Add, Subtract, Multiply, Divide, Modulo, Math.Round, Math.Min, Math.Max
**Logic (7):** If, And, Or, Not, Equals, GreaterThan, LessThan
**String (7):** String.Concat, String.Contains, String.Split, String.Replace, String.Length, String.ToNumber, String.FromObject
**List (9):** List.Create, List.GetItemAtIndex, List.Count, List.FirstItem, List.Flatten, List.FilterByBoolMask, List.Range, List.Sort, List.UniqueItems
**Dictionary (2):** Dictionary.ByKeysValues, Dictionary.ValueAtKey
**Color (2):** Color Picker, Color.ByARGB
**DateTime (2):** DateTime.Now, DateTime.Format
**File (4):** CSV.ReadFromFile, CSV.WriteToFile, Text.ReadFromFile, Text.WriteToFile
**Geometry (3):** Point.ByCoordinates, Point.Components, BoundingBox.Center

**Navisworks.Document (3):** Document.Current, Document.Info, Document.Models
**Navisworks.Model (2):** Model.RootItem, Models.RootItems
**Navisworks.ModelItem (5):** ModelItem.Children, ModelItem.Descendants, ModelItem.DisplayName, ModelItem.HasGeometry, ModelItem.BoundingBox
**Navisworks.Properties (2):** Properties.Value, Properties.Categories
**Navisworks.Search (2):** Search.ByPropertyValue, Search.ByPropertyContains
**Navisworks.Selection (3):** Selection.Current, Selection.SetCurrent, Selection.Clear
**Navisworks.SelectionSets (4):** SelectionSets.All, SelectionSet.ByName, SelectionSet.Items, SelectionSet.Create
**Navisworks.Appearance (5):** Appearance.OverrideColor, Appearance.OverrideTransparency, Appearance.Reset, Appearance.Hide, Appearance.Show
**Navisworks.Viewpoints (4):** Viewpoints.All, SavedViewpoint.ByName, SavedViewpoint.Apply, Viewpoint.SaveCurrent
**Navisworks.Clash (5):** Clash.Tests, ClashTest.Info, ClashTest.Results, ClashResult.Info, ClashResult.Items, ClashResult.Center — (6)

Notes for implementers: all MVP nodes except the Input/Output/List.Create/Color Picker NodeModels are zero-touch static methods; Document ports default to Application.ActiveDocument when unwired; List<ModelItem> is the universal item currency; MVP total = 88 (62 core + 26 Navisworks).
