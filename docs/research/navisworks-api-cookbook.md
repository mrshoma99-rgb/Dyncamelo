...
**Verification method**: All member signatures below were dumped from the actual `Speckle.Navisworks.API 2024.0.0` assemblies (`Autodesk.Navisworks.Api.dll`, `.Clash.dll`, `.ComApi.dll`) and `Chuongmep.Navis.Api.Autodesk.Navisworks.Timeliner 2023.0.7` (`Autodesk.Navisworks.Timeliner.dll`) using a `MetadataLoadContext` (net8 + `System.Reflection.MetadataLoadContext` 8.0.0, resolver = lib/net48 dirs + `Microsoft.NETFramework.ReferenceAssemblies.net48` ref pack). (The dump files themselves were produced in a temporary build workspace and are not checked in; the dumper is trivial to reproduce with `MetadataLoadContext` over the NuGet `lib/net48` folders.) **Every signature in this report is verified against those dumps unless explicitly marked UNVERIFIED (docs-only).** Only 3 items are docs-only: plugin folder discovery rules, threading rules, and the 2023-vs-2024 Timeliner note.
